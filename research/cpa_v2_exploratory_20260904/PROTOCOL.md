# Authorized exploratory amendment — registered before IS

This section takes precedence over conflicting original data/visual gate text
retained below. User instruction: “Forget data problems, go ahead to finish
backtest and pick a good strategy”. The original blocked study is preserved.

Classification: EXPLORATORY_UNVERIFIED_CPA_V2. Data quality remains failed and
visual acceptance remains false. This study evaluates the current numerical
CPA_v2 model; it does not claim certified replication of Kell's examples.
Source research and the existing visual review remain available.

Data policy, fixed before any IS performance:
- Raw Tradier responses and finite prices are used as supplied, including
  inconsistent OHLC ranges. No vendor corrections or other source is invented.
- Use the original SPY calendar and original 1508/503/503 partition boundaries.
  Missing symbol rows are reindexed onto that calendar with NaNs.
- A signal requires all five current OHLCV fields finite and positive prices.
  Missing rows cannot produce a signal. Rolling feature windows containing
  missing data remain unavailable where the implementation requires full windows.
  Weekly aggregates use available observations and can be incomplete; disclose.
- An unavailable next-session open cancels an entry. Sell/trim orders defer
  to an available open; a pending full exit takes precedence over new orders.
- No forward-filled price is used for a signal or fill. A missing close uses
  only the most recent finite close for mark-to-market; explicitly flag stale
  valuation days. Cash days remain in every risk metric's denominator.
- Benchmark enters on the first available open. Terminal liquidation requires
  a finite supplied final close. All seven terminal closes are checked pre-IS.
- Metrics are net of modeled costs but reflect vendor price returns, not
  guaranteed total returns. Missing/stale bars can bias risk metrics.

All existing IS selection, neighboring-parameter rejection, Validation gates,
OOS return/Sharpe/20-cycle requirements and once-only controls remain in force.
No extra candidate is promoted after seeing Validation. All 256 original
configurations are tested on all eight requested pairs; all outputs retained.

Predeclared reporting: show best IS configuration for each pair descriptively,
including rejected configurations, without promoting them to Validation/OOS.
Benchmarks for all seven assets are evaluated in the one terminal OOS batch,
even if no strategy survives. Empty survivor sets imply no selectable strategy;
never run rejected IS candidates on OOS to manufacture a recommendation.

An existing reference/missing-data implementation test is allowed before IS;
no real-data performance tuning has been done at registration.
