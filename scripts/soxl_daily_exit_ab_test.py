...
def prep(x):
    ...
    tp=(x.high+x.low+x.close)/3; ma=tp.rolling(CCI_N).mean(); md=tp.rolling(CCI_N).apply(lambda z:np.mean(np.abs(z-np.mean(z))),raw=True); x['cci']=(tp-ma)/(0.015*md); x['prev_high']=x.high.shift(1); return x
...
def bt(seg,mode):
    ...
    for dt,r in x.iterrows():
        ...
        if pd.notna(r.wr) and pd.notna(r.cci):
            entry=(r.wr<WE) and (r.cci<CE)
            base_exit=(r.close>r.prev_high) or (r.wr>WX)
            exit_sig=base_exit if mode=='A_original' else (base_exit or (r.cci>CX))
    ...
    return {'exit_mode':mode,'trades':len(t),'win_rate':win,'profit_factor':pf,'avg_trade':avg,'median_trade':med,'avg_hold_days':hold,'total_return':total,'cagr':cagr,'sharpe':sharpe,'max_drawdown':mdd,'calmar':calmar,'exposure':e.pos.mean(),'ending_equity':e.equity.iloc[-1]}
...
def main():
    ...
    for p,seg in segs.items():
        for mode in ['A_original','B_plus_CCI_exit']:
            z=bt(seg,mode); z.update(period=p,start=seg.index.min().date(),end=seg.index.max().date(),execution='next_open'); rows.append(z)
    pd.DataFrame(rows).to_csv(OUT/'soxl_daily_exit_A_vs_B.csv',index=False); print(pd.DataFrame(rows).to_string(index=False))
