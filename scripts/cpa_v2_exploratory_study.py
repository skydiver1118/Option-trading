...
import numpy as np
import pandas as pd
from cpa_v2_exploratory_engine import Parameters,SPACE,configurations,neighbors,detect,simulate,metrics

ROOT=Path(__file__).resolve().parents[1]
STUDY=ROOT/'research/cpa_v2_exploratory_20260904'
RAW=ROOT/'data/cpa_v2_20260904_source'
OUT=STUDY/'results'
PAIRS=[('SPY','SPY'),('SPMO','SPMO'),('VGT','VGT'),('SMH','SMH'),
       ('QQQ','QQQ'),('TQQQ','TQQQ'),('SMH','SOXL'),('QQQ','TQQQ')]
CODE=[ROOT/'scripts/cpa_v2_exploratory_engine.py',Path(__file__),STUDY/'PROTOCOL.md',STUDY/'SOURCE_DEFINITIONS.md',STUDY/'authorization.json',STUDY/'visual_review.json']
...
def prepare():
    ...
    for symbol in sorted(set(sum(([s,t] for s,t in PAIRS),[]))):
        ...
        for stage,part in parts.items():
            ...
            # Preserve all common sessions; absent bars stay NaN, never fabricated OHLCV.
            x.reindex(spy.index).loc[:part['end']].to_csv(dest,index_label='date')
...
def gate():
    ...
    amendment=read_json(STUDY/'authorization.json')
    if amendment.get('data_limitations_accepted') is not True:
        raise RuntimeError('User data-limitation authorization missing')
    # Do not falsify source integrity or visual acceptance. This is a separately
    # registered experimental implementation evaluation, not certified Kell replication.
    if amendment.get('classification')!='EXPLORATORY_UNVERIFIED_CPA_V2':
        raise RuntimeError('Exploratory classification missing')


DETECTION_CACHE={}
PRICE_CACHE={}

def cached_price(symbol,stage):
    key=(symbol,stage)
    if key not in PRICE_CACHE:PRICE_CACHE[key]=load_stage(symbol,stage)
    return PRICE_CACHE[key]

def cached_detect(symbol,stage,p):
    key=(symbol,stage,p.id)
    if key not in DETECTION_CACHE:DETECTION_CACHE[key]=detect(cached_price(symbol,stage),p)
    return DETECTION_CACHE[key]

def case(p,s,t,stage,save=True,stress=False):
    part=read_json(OUT/'partitions.json')[stage]
    execution=cached_price(t,stage)
    events=cached_detect(s,stage,p)
...
def run_is():
    ...
    for s,t in PAIRS:
        rows=[]
        print('IS screening',s,'->',t,flush=True)
        for index,p in enumerate(configurations()):
            m=case(p,s,t,'IS');row=dict(signal=s,trade=t,id=p.id,kind='initial',**m)
            allrows.append(row);rows.append(row)
            if (index+1)%64==0:print('IS progress',s,t,index+1,flush=True)
        pd.DataFrame(rows).to_csv(OUT/f'IS_{s}_{t}_screen.csv',index=False)
...
def run_oos():
    ...
    try:
        ...
        if True:  # Predeclared benchmark report for the entire requested universe.
