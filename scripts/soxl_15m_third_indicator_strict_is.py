...
def prep():
 x=fetch15('SOXL'); q=fetch15('QQQ')[['close','volume']].rename(columns={'close':'qclose','volume':'qvol'}); x=x.join(q,how='inner'); hh=x.high.rolling(WR_N).max(); ll=x.low.rolling(WR_N).min(); x['wr']=-100*(hh-x.close)/(hh-ll); tp=(x.high+x.low+x.close)/3; ma=tp.rolling(CCI_N).mean(); md=tp.rolling(CCI_N).apply(lambda z:np.mean(np.abs(z-np.mean(z))),raw=True); x['cci']=(tp-ma)/(0.015*md); x['prev_high']=x.high.shift(1); x['gap']=x.open/x.close.shift(1)-1; x['clv']=(2*x.close-x.high-x.low)/(x.high-x.low).replace(0,np.nan)
