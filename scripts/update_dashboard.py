...
def tradier_expirations(symbol):
    j=tradier_get('/markets/options/expirations',{'symbol':symbol,'includeAllRoots':'false','strikes':'false'})
    dates=((j or {}).get('expirations') or {}).get('date') or []
    if isinstance(dates,str): dates=[dates]
    return list(dates)


def yfinance_expirations(symbol):
...
def tradier_put_rows(ticker,expiry):
    j=tradier_get('/markets/options/chains',{'symbol':ticker,'expiration':expiry,'greeks':'true'})
    options=((j or {}).get('options') or {}).get('option') or []
    ...
    for o in options:
        ...
        g=o.get('greeks') or {}
        iv=g.get('mid_iv')
        if iv is None: iv=g.get('smv_vol')
        delta=g.get('delta')
...
def yfinance_put_rows(ticker,expiry):
    ...
    for _,r in puts.iterrows():
        rows.append({'strike':r.strike,'bid':r.bid,'ask':r.ask,'iv':r.impliedVolatility if pd.notna(r.impliedVolatility) else None,'delta':None,'source':'yfinance','delta_source':None})
    return rows
...
def candidate_puts(ticker,price,support,expiry):
    ...
    if TRADIER_TOKEN:
        try:
            ...
            print(f'Tradier chain fallback for {ticker}: {e}')
            rows=yfinance_put_rows(ticker,expiry); source='yfinance'
    else:
        rows=yfinance_put_rows(ticker,expiry)
    ...
    for r in rows:
        try:
            strike=float(r.get('strike')); bid=float(r.get('bid') or 0); ask=float(r.get('ask') or 0)
        except Exception: continue
        if strike>=price: continue
        mid=(bid+ask)/2 if bid>0 and ask>0 else max(bid,ask)
        if mid<=0: continue
        iv=r.get('iv')
        try: iv=float(iv) if iv is not None else None
...
def should_run(now):
    ...
    if off==-4:
        valid={
            '20 14 * * 1-5','30 14 * * 1-5','40 14 * * 1-5',
            '0 17 * * 1-5','15 17 * * 1-5',
            '30 18 * * 1-5','45 18 * * 1-5'
        }
    else:
        valid={
            '20 15 * * 1-5','30 15 * * 1-5','40 15 * * 1-5',
            '0 18 * * 1-5','15 18 * * 1-5',
            '30 19 * * 1-5','45 19 * * 1-5'
        }
...
def main():
    ...
    for sym in PUT_NAMES:
        x=raw[sym]; expiry,_=choose_expiry(sym); candidates,source=candidate_puts(sym,x['price'],x['key_support'],expiry); option_sources.add(source)
        ...
        analyses.append({'ticker':sym,'price':f(x['price']),'change_pct':f(x['change_pct']),'decision':decision,'score':int(score),'contract':(f"{expiry} ${pref['strike']:.0f}P" if pref else None),'premium':f(pref['premium']) if pref else None,'breakeven':f(pref['breakeven']) if pref else None,'key_support':f(x['key_support']),'fib':{k:f(v) for k,v in x['fib'].items()},'candidates':clean,'option_source':source,'trigger':f"Nearest technical support ${x['key_support']:.2f}; prefer stable/reclaiming tape before entry.",'risk':'3x daily leverage and volatility drag.' if sym=='SOXL' else ('Earnings falls before expiration.' if event else 'Gap risk and volatility expansion.'),'note':f"Adjusted 45-day swing ${x['low45']:.2f} → ${x['high45']:.2f}; EMA20 ${x['ema20']:.2f}"})
    ...
    source_label='Tradier real-time' if option_sources=={'Tradier'} else ('yfinance fallback' if option_sources=={'yfinance'} else 'Tradier + yfinance fallback')
    payload={'updated_et':now.strftime('%Y-%m-%d %I:%M %p ET'),'market_state':'OPEN' if market_open_now(now) else 'CLOSED','option_data_source':source_label,'tickers':[{'ticker':k,'price':f(v['price']),'change_pct':f(v['change_pct'])} for k,v in raw.items()],'ranking':ranking,'analysis':analyses,'smh':{'price':f(smh.get('price')),'entries':entries}}
