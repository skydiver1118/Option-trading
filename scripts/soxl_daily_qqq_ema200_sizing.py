#!/usr/bin/env python3
# QQQ EMA200 position-sizing overlay backtest; fixed daily SOXL WR2/CCI5 rule.
...
def bt(x,bear_weight):
 cash=100000.; sh=0.; pos=False; pending=None; ent=None; entdt=None; allocated=0.; trs=[]; eq=[]
 for dt,r in x.iterrows():
  if pending:
   side,weight=pending; px=float(r.open)
   if side=='buy' and not pos:
    allocated=cash*weight; sh=allocated/px; cash-=allocated; ent=px; entdt=dt; pos=True
   elif side=='sell' and pos:
    cash+=sh*px; trs.append((entdt,dt,px/ent-1,allocated/100000.)); sh=0.; allocated=0.; pos=False
   pending=None
  if pd.notna(r.wr) and pd.notna(r.cci):
   if pos and ((r.close>r.prev_high) or (r.wr>WX) or (r.cci>CX)): pending=('sell',0)
   elif not pos and r.wr<WE and r.cci<CE:
    weight=1.0 if r.qqq_close>r.ema200 else bear_weight; pending=('buy',weight)
 ...
 return {'bear_weight':bear_weight,'trades':len(t),'win_rate':win,'profit_factor':pf,'avg_trade':avg,'total_return':total,'cagr':cagr,'sharpe':sharpe,'max_drawdown':mdd,'calmar':calmar,'exposure':e.pos.mean(),'ending_equity':e.equity.iloc[-1]}
...
def main():
 ...
 for p,seg in segs.items():
  for w in [0.0,0.25,0.5,0.75,1.0]:
   z=bt(seg,w); z.update(period=p,start=seg.index.min().date(),end=seg.index.max().date()); rows.append(z)
 pd.DataFrame(rows).to_csv(OUT/'soxl_daily_qqq_ema200_sizing_compare.csv',index=False); print(pd.DataFrame(rows).to_string(index=False))
