...
def simulate(execution,events,start,end,cost=0.0005,benchmark=False):
    ...
    ledger=[];fills=[];trades=[]
    for j,(date,r) in enumerate(ex.iterrows()):
        before_open=cash+shares*r.open
        fees_today=0.;turnover=0.
        if benchmark and j==0:pending=(1.,'BUY_HOLD',None,-1)
        ...
        ledger.append(dict(date=date,equity=equity,return_daily=equity/previous-1,
                           ...
                           turnover=turnover/previous))
        previous=equity
        if j<len(ex)-1 and not benchmark:
            row=ev.iloc[j]
            if pd.notna(row.signal_target):
