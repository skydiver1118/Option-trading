...
def run(d,entry,exit_rule,start,end):
 ...
 for i,(dt,row) in enumerate(x.iterrows()):
  if pos and e['exit']:
   px=float(row.open);rr=px/e['px']-1;eq*=1+rr;ts.append(rr);pos=False;e=None
...
d=get(S);q=get('QQQ').reindex(d.index).ffill();
# cache indicators
WR={n:wr(d,n) for n in range(2,11)};CCI={n:cci(d,n) for n in range(3,16)}
# Phase 1: optimize WR+CCI entry on IS only; exit uses corresponding WR > -30 or close > prior high.
base=[]
...
base_sh=max(base,key=lambda z:(z[0],z[1]));base_ret=max(base,key=lambda z:(z[1],z[0]))
# Use IS-Sharpe base for third-indicator tournament (predeclared objective)
_,_,wn,wt,cn,ct,bis=base_sh;core=(WR[wn]<wt)&(CCI[cn]<ct);ex=(d.close>d.high.shift(1))|(WR[wn]>-30)
# candidate third filters
...
for n in [10,20,50,100,200]:
 ema=d.close.ewm(span=n,adjust=False).mean();dist=d.close/ema-1
 for th in [-.2,-.15,-.1,-.05,0,.05]:cands.append((f'TQQQ_EMA{n}_DIST_GE_{th}','trend',core&(dist>=th)))
for n in [20,50,100,200]:
 ema=q.close.ewm(span=n,adjust=False).mean();dist=q.close/ema-1
