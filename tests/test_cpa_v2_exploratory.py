...
from cpa_v2_exploratory_engine import Parameters,features,detect,simulate,metrics,configurations
...
class MissingDataTests(unittest.TestCase):
    def test_missing_next_open_cancels_entry(self):
        x=bars(5);x.loc[x.index[1],'open']=np.nan
        e=signals(x.index,[(0,1.,'WEDGE_POP')])
        lg,tr,fl=simulate(x,e,x.index[0],x.index[-1])
        self.assertEqual(len(fl),0)
        self.assertTrue((lg.equity==1).all())

    def test_missing_exit_open_defers_exit_and_marks_stale_close(self):
        x=bars(6);x['open']=100.;x['close']=100.
        x.loc[x.index[2],['open','close']]=np.nan
        x.loc[x.index[3],['open','close']]=80.
        e=signals(x.index,[(0,1.,'WEDGE_POP'),(1,0.,'WEDGE_DROP')])
        lg,tr,fl=simulate(x,e,x.index[0],x.index[-1],cost=0)
        self.assertAlmostEqual(lg.equity.iloc[2],1.)
        self.assertAlmostEqual(lg.equity.iloc[-1],.8)
        self.assertEqual(fl.date.iloc[-1],x.index[3])
        self.assertEqual(fl.signal_date.iloc[-1],x.index[1])
        self.assertTrue(lg.stale_close.iloc[2])

    def test_missing_current_bar_cannot_signal(self):
        x=bars(650);x.iloc[350]=np.nan
        a=detect(x,Parameters())
        self.assertEqual(a.cpa_event.iloc[350],'')
        self.assertFalse(a.features_ready.iloc[350])
        pd.testing.assert_frame_equal(a.iloc[:401],detect(x.iloc[:401],Parameters()),check_freq=False)

    def test_optimized_iteration_matches_original_on_complete_reference(self):
        from cpa_v2_engine import detect as old_detect, simulate as old_simulate
        x=bars(650);p=Parameters()
        pd.testing.assert_frame_equal(detect(x,p),old_detect(x,p))
        e=signals(x.index,[(110,1.,'WEDGE_POP'),(120,.5,'EXHAUSTION_EXTENSION_2'),(130,0.,'WEDGE_DROP')])
        a=simulate(x,e,x.index[0],x.index[-1]);b=old_simulate(x,e,x.index[0],x.index[-1])
        pd.testing.assert_frame_equal(a[0][b[0].columns],b[0])
        pd.testing.assert_frame_equal(a[1],b[1]);pd.testing.assert_frame_equal(a[2],b[2])
